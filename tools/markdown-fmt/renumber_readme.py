...
def renumber_headers(file_path):
    ...
    numbering_pattern = re.compile(r"^#+\s+(\d+(\.\d+)*\.?)\s+")  # noqa: F841
    ...
    for line in lines:
        ...
        # 4칸 이상 indent된 라인은 코드 블록으로 간주 (Markdown indented code block)
    ...
    if original_content != new_content:
        ...
        fd, tmp_path = tempfile.mkstemp(dir=str(parent), suffix=".md")
        try:
